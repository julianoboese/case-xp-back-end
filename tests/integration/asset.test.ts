...
describe('The GET /assets route', () => {
  ...
  it('returns all user assets if the request is successful', async () => {
    ...
    expect(result.statusCode).toBe(200);
    expect(result.body).not.toEqual(expect.arrayContaining([expect.objectContaining({
      ticker: 'CASH3',
    })]));
    expect(result.body).toEqual(expect.arrayContaining([expect.objectContaining({
      ticker: 'PRIO3',
...
describe('The GET /assets/{assetId} route', () => {
  ...
  it('returns an error if the asset is unavailable at the broker', async () => {
    ...
    const result = await request(server).get('/assets/1000')
      .set('Authorization', loginResult.body.token);
  ...
  it('returns a specific asset from the broker if the user doesn\'t own it', async () => {
    ...
    const result = await request(server).get('/assets/348')
      .set('Authorization', loginResult.body.token);
