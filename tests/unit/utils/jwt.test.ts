...
describe('The Jwt "authenticateToken" function', () => {
  ...
  it('should throw an error if token is not provided', async () => {
    await expect(Jwt.authenticateToken())
  ...
  it('should return user data if token is verified', async () => {
    ...
    await expect(Jwt.authenticateToken('t5r4e3w2q1'))
    .rejects.toThrow('Acesso não autorizado.')
  });

  it('should return user data if token is verified', async () => {
    jest.spyOn(jwt, 'verify').mockImplementation(() => ({id: 1, email: 'felipe.silva@email.com'}));
